...
public class EditorButton : Editor
{
    ...
    {
        var ab = parameterInfo.ParameterType.GetCustomAttributes(true);
        if (ab != null)
        {
            object fa = ab.Where(a => a.GetType() == typeof(FlagsAttribute)).FirstOrDefault();
            if (fa != null)
            {
                return EditorGUILayout.EnumFlagsField((Enum)val);
            }
        }
